...
module plexi::vault_v2 {
    ...
    public entry fun init_vault(
        ...
        let vault_state = VaultState {
            ...
            total_shares: 0,
            is_initialized: true,
            usdc_coins: coin::zero<AptosCoin>(),
    ...
    /// Register USDC coin for a user (required before deposits)
    ...
    public entry fun user_deposit(
        ...
        let usdc_payment = coin::withdraw<AptosCoin>(user, amount);
        let vault_state = borrow_global_mut<VaultState>(user_addr);
        coin::merge(&mut vault_state.usdc_coins, usdc_payment);
    ...
    public entry fun user_withdraw(
        ...
        let vault_state = borrow_global_mut<VaultState>(user_addr);
        let usdc_payment = coin::extract(&mut vault_state.usdc_coins, amount);
        coin::deposit(user_addr, usdc_payment);
    }
    ...
    public entry fun user_mint(
        ...
        // Transfer USDC from user to vault
        let usdc_payment = coin::withdraw<AptosCoin>(user, usdc_amount);
        coin::merge(&mut vault_state.usdc_coins, usdc_payment);
